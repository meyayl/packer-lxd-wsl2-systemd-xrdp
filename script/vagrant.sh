...
if [ "$INSTALL_VAGRANT_KEY" = "true" ] || [ "$INSTALL_VAGRANT_KEY" = "1" ]; then
    ...
    if ! id -u $SSH_USER >/dev/null 2>&1; then
      default_user=$(id 1000 --user --name)
      default_group=$(id 1000 --group --name)
      echo "==> Delete prexisting user ${default_user} with default group ${default_group} and home folder"
      deluser ${default_user}
      delgroup ${default_group} || true
      rm -rf /home/${default_user}

      echo "==> Creating $SSH_USER user"
      /usr/sbin/groupadd $SSH_USER
      /usr/sbin/useradd $SSH_USER -g $SSH_USER -G sudo -d $SSH_USER_HOME --create-home --shell /bin/bash
